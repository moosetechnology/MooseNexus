...
MooseNexus passes `--allow-path-traversal` to `oras pull`. This is needed because the bundle is pushed from generated absolute staging paths, and ORAS otherwise refuses to write layers whose recorded names are outside the pull output directory. The pull output directory is generated by MooseNexus unless explicitly provided by the caller.
